    ...
    <div className="w-full h-screen overflow-y-auto bg-white p-8 scroll-smooth">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-12">
          <div className="w-32 h-32 rounded-full bg-yellow-500 flex items-center justify-center text-5xl font-black text-white mb-4 border-4 border-white shadow-xl">
             {firstLetter}
          </div>
          <h1 className="text-4xl font-bold tracking-tight">{user?.username || 'Pinterest User'}</h1>
          <p className="text-zinc-500 mt-2 font-medium">@{user?.username?.toLowerCase().replace(/\s+/g, '') || 'user'}</p>
          
          <div className="flex gap-4 mt-8">
            <button className="bg-zinc-100 px-6 py-3 rounded-full font-bold hover:bg-zinc-200 transition-all active:scale-95">Edit Profile</button>
            <button className="bg-zinc-100 px-6 py-3 rounded-full font-bold hover:bg-zinc-200 transition-all active:scale-95">Share</button>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex justify-center gap-8 mb-12 border-b border-zinc-100 pb-2">
            <button 
                onClick={() => setActiveTab('created')}
                className={`pb-4 text-lg font-bold transition-all relative ${activeTab === 'created' ? 'text-black' : 'text-zinc-400 hover:text-zinc-600'}`}
            >
                Created
                {activeTab === 'created' && <div className="absolute bottom-0 left-0 w-full h-1 bg-black rounded-full" />}
            </button>
            <button 
                onClick={() => setActiveTab('saved')}
                className={`pb-4 text-lg font-bold transition-all relative ${activeTab === 'saved' ? 'text-black' : 'text-zinc-400 hover:text-zinc-600'}`}
            >
                Saved
                {activeTab === 'saved' && <div className="absolute bottom-0 left-0 w-full h-1 bg-black rounded-full" />}
            </button>
        </div>
        ...
        <div className="flex justify-between items-center mb-10">
            <div className="flex items-center gap-2">
                <button className="p-3 hover:bg-zinc-100 rounded-full transition-colors">
                    <span className="text-xl">🎚️</span>
                </button>
            </div>
            <button className="w-12 h-12 rounded-full hover:bg-zinc-100 flex items-center justify-center text-3xl font-light transition-colors">+</button>
        </div>
        ...
        {activeTab === 'saved' ? (
            ...
            {boards.map((board) => (
                ...
                <div className="h-56 rounded-[32px] overflow-hidden bg-zinc-100 mb-4 shadow-sm group-hover:shadow-xl transition-all duration-300 relative">
                ...
                <div className="px-2">
                    <h3 className="font-bold text-xl group-hover:text-red-600 transition-colors">{board.name}</h3>
